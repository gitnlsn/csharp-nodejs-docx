...
describe("Update Table of Contents", () => {
    it("should insert a new section in the table of contents", async () => {
        ...
        if (resultObject.Error) {
            console.error("Error:", resultObject.Error);
            console.error("Stack Trace:", resultObject.StackTrace);
            throw new Error(resultObject.Error);
        }

        // Save the modified document for inspection
        fs.writeFileSync(path.join(__dirname, "modified-toc-sample.docx"),
            Buffer.from(resultObject.ModifiedDocument, 'base64'));

        // Verify the result contains our success message
        expect(resultObject.Message).toBe("Table of Contents updated successfully");
    });
}); 