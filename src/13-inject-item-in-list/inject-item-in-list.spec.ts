...
describe("Inject Item In List", () => {
    it("should add a new item to the numbered list in the 'In Scope' section", async () => {
        ...
        const listItems = resultObject.ListItems;

        // Save the modified document
        fs.writeFileSync(path.join(__dirname, "modified-sample.docx"),
            Buffer.from(resultObject.ModifiedDocument, 'base64'));

        // The last item should be our newly added Lorem Ipsum text
        expect(listItems.length).toBeGreaterThan(1);
        expect(listItems[listItems.length - 1]).toContain("Lorem ipsum dolor sit amet");
    });
});