...
describe("Extract Paragraphs Between Sections", () => {
    it("should extract and replace only the specific paragraph", async () => {
        ...
        // Send the base64 encoded document to the C# script
        ...
        const originalParagraphs = resultObject.OriginalParagraphs;
        const modifiedParagraphs = resultObject.ModifiedParagraphs;

        // Save the modified document for inspection
        fs.writeFileSync(path.join(__dirname, "modified-sample.docx"),
            Buffer.from(resultObject.ModifiedDocument, 'base64'));

        // Verify that all original paragraphs were extracted correctly
        expect(originalParagraphs).toEqual([
        ...
        // Verify only the target paragraph was modified
        expect(modifiedParagraphs[1]).not.toEqual(originalParagraphs[1]);
        expect(modifiedParagraphs[0]).toEqual(originalParagraphs[0]);
        expect(modifiedParagraphs[2]).toEqual(originalParagraphs[2]);
    });
});